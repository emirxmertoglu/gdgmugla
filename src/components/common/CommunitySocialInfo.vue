<template>
  <p class="google-font mt-4 mb-3" style="font-size: 130%">
    ...
    <v-tooltip
      top
      v-if="checkExistance(config.generalConfig.socialLinks.kommunity, 0)"
    >
      <template v-slot:activator="{ on }">
        <v-btn
          text
          ...
          v-on="on"
          rel="noreferrer"
          :href="config.generalConfig.socialLinks.kommunity"
          target="_blank"
          slot="activator"
        >
    ...
    <v-tooltip
      top
      v-if="checkExistance(config.generalConfig.socialLinks.meetup, 0)"
    >
      <template v-slot:activator="{ on }">
        <v-btn
          text
          ...
          v-on="on"
          rel="noreferrer"
          :href="config.generalConfig.socialLinks.meetup"
          target="_blank"
          slot="activator"
        >
